...
import static com.yifan.butterfly.C.ILLEGALSTATE_EXCEPTION;
...
public final class ButterflyProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnv) {
        ...
        for (Element activity : roundEnv.getElementsAnnotatedWith(BActivity.class)) {
            ...
            model._Result = annotation.result();
        ...
        for (ButterflyModel model : activityModels) {
            ...
            MethodSpec.Builder helperConstructor = MethodSpec.constructorBuilder()
                    .addModifiers(PUBLIC)
                    .addParameter(CONTEXT, "from")
                    .addStatement("super(from)");
            helper.addMethod(helperConstructor.build());
            ...
            MethodSpec.Builder start = MethodSpec.methodBuilder("start")
                    .addAnnotation(Override.class)
                    .addModifiers(PUBLIC)
                    .addStatement("_intent.setComponent(new $T(_context, $T.class))",
                            COMPONENT_NAME, activityClassName)
                    .addStatement("_context.startActivity(_intent, _options)");
            ...
            MethodSpec.Builder start_for_result = MethodSpec.methodBuilder("startForResult")
                    ...
                    .addStatement("if(!(_context instanceof $T)) throw new $T(\"startForResult must be called within Activity!\")",
                            ACTIVITY, ILLEGALSTATE_EXCEPTION)
                    .addStatement("_intent.setComponent(new $T(_context, $T.class))",
                            COMPONENT_NAME, activityClassName)
                    .addStatement("(($T)_context).startActivityForResult(_intent, requestCode, _options)", ACTIVITY);
            ...
            MethodSpec.Builder getHelper = MethodSpec.methodBuilder("get" + helperName.simpleName())
                    ...
                    .addParameter(CONTEXT, "from")
                    .returns(helperName)
                    .addStatement("return new $T($L)", helperName, "from");
